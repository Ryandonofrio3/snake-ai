# solution_v2_final.py
# The definitive, stable version of the advanced XGBoost Snake AI.
...
# Import SnakeEnv to run our own game loop for better control
...
def get_config():
    ...
    return {
        ...
        # --- Curriculum Learning ---
        "CURRICULUM_STAGES": [
            {'board_size': (6, 6), 'threshold': 10},
            {'board_size': (8, 8), 'threshold': 13},
            {'board_size': (10, 10), 'threshold': 100},
        ],
        ...
        "REWARD_STAGNATE": -0.02, "REWARD_SURVIVAL": 0.001,
        "REWARD_WALL_PENALTY": -0.015,

        # --- XGBoost Hyperparameters ---
        "XGB_PARAMS": {
            'objective': 'multi:softprob',
            'num_class': 3, 'learning_rate': 0.12, 'n_estimators': 400,
            'max_depth': 7, 'subsample': 0.85, 'colsample_bytree': 0.85,
            'reg_alpha': 0.15, 'reg_lambda': 1.2, 'min_child_weight': 3,
            'random_state': 42, 'tree_method': 'hist', 'eval_metric': 'mlogloss',
            'verbosity': 0
        }
    }
...
class RewarderV2:
    """
    A unified, stateful reward calculator that adapts to the curriculum.
    """
    ...
    def calculate(self, env, ate_food, is_alive):
        # On the first, smallest board, use the simplest possible reward signal
        if env.width <= 6:
            if ate_food: return 1.0
            if not is_alive: return -1.0
            return 0.0

        # On larger boards, use the advanced, shaped rewards
        if not is_alive:
...
class AdvancedXGBoostTrainer:
    """
    Final trainer with Pure Imitation Bootstrap and curriculum learning.
    """
    ...
    def _create_golden_episode(self):
        self.log_func("Injecting a 'Golden Episode' to bootstrap the learning process...")
        env = SnakeEnv(6, 6)
    ...
    def _play_single_episode(self):
        ...
        rewarder = RewarderV2(self.config); rewarder.reset()
        while not done:
    ...
    def train_iteration(self, iteration_num):
        ...
        if not episodes:
            self.log_func("Warning: No episodes collected."); return {'best_score': 0, 'mean_score': 0}
